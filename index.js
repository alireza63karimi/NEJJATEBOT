// ============================================================================
// index.js - NEJJATEBOT Ultra-Fast VIP (2025)
...
// ============================================================================
let BOT_TOKEN = null;
if (fs.existsSync(TOKEN_PATH)) BOT_TOKEN = fs.readFileSync(TOKEN_PATH, 'utf8').trim();
if (!BOT_TOKEN) { console.error("❌ BOT_TOKEN پیدا نشد! Secret File بسازید."); process.exit(1); }
...
async function tg(method, body) {
  try { const res = await fetch(API(method), { method:'POST', headers:{'content-type':'application/json'}, body: JSON.stringify(body||{}) }); return res.json(); }
  catch (e) { console.error("tg error:", e.message); return null; }
}
async function sendMessage(chat_id, text, extra={}) { try { return await tg("sendMessage",{chat_id,text: String(text),parse_mode:"HTML",...extra}); } catch(e){console.error("sendMessage error:",e.message);} }
async function answerCallback(id, text=""){ if(!id) return; try{ await tg("answerCallbackQuery",{callback_query_id:id,text}); } catch{} }

// ============================================================================
// KEYBOARDS
// ============================================================================
const contactKeyboard = ()=>({ reply_markup:JSON.stringify({ keyboard:[[ { text:"ارسال شماره تماس 📞", request_contact:true } ]], resize_keyboard:true, one_time_keyboard:true }) });
const adminInlineKeyboard = ()=>({ reply_markup:JSON.stringify({ inline_keyboard:[
...
async function resolveChannelId(input){
  ...
  if(typeof input==="string" && input.startsWith("@")){ const info = await tg("getChat",{chat_id:input}); return info?.ok? info.result.id:null; }
...
function scheduleQueue(){ while(inviteQueue.length && activeWorkers<MAX_WORKERS){ activeWorkers++; processInvite(inviteQueue.shift()); } }
...
app.post("/webhook",async(req,res)=>{
  const update=req.body; if(!update) return res.sendStatus(200);
  ...
  try{
    if(update.message){
      ...
      if(msg.text==="/start"){ await sendMessage(chatId,config.welcomeMessage); await sendMessage(chatId,"شماره تماس را ارسال کنید:",contactKeyboard()); return; }
      if(msg.text==="/admin"){ if(!isAdmin(userId)) return sendMessage(chatId,"⛔ دسترسی ندارید"); return sendMessage(chatId,"پنل مدیریت:",adminInlineKeyboard()); }

      if(msg.contact){ users[userId].phone=msg.contact.phone_number; saveUsers(); return sendMessage(chatId,config.agreementText,{reply_markup:JSON.stringify({inline_keyboard:[[{text:config.agreementButton,callback_data:"AGREE"}]]})}); }
